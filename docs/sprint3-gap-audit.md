...
|-------|-------|--------|----------|---------------|---------------|-------------|
| UC-074 | Interview Question Bank | ✅ DONE | `src/pages/QuestionBank.tsx` (381 lines)<br>`question_bank_items` table with RLS<br>Category/difficulty/search filters | None | ⚠️ No dedicated test file | Create `src/test/sprint3/uc074-question-bank.test.tsx` |
| UC-075 | Question Practice Tracking | ✅ DONE | `src/pages/QuestionPractice.tsx` (410 lines)<br>`question_practice_responses` table<br>Time tracking, draft/submit status<br>`QuestionPracticeHistory.tsx` component | None | ⚠️ No dedicated test file | Create `src/test/sprint3/uc075-practice-tracking.test.tsx` |
| UC-076 | Mock Interview Sessions | ✅ DONE | `src/pages/MockInterviewSession.tsx`<br>`src/pages/MockInterviewSummary.tsx`<br>`mock_interview_sessions`, `mock_interview_responses`, `mock_interview_summaries` tables<br>`MockInterviewSetup.tsx` component (190 lines) | None | ⚠️ Partial in `analytics-calculations.test.ts` | Create comprehensive `src/test/sprint3/uc076-mock-interviews.test.tsx` |
| UC-077 | AI Interview Feedback | ⚠️ PARTIAL | `supabase/functions/ai-question-feedback/index.ts`<br>`question_practice_feedback` table<br>`QuestionPracticeFeedback.tsx` (354 lines) | **Uses rules-based fallback** instead of AI when Lovable AI unavailable | ⚠️ Fallback logic not tested | Add test for rules-based scoring in `src/test/sprint3/uc077-ai-feedback.test.tsx` |
| UC-078 | Interview Analytics | ✅ DONE | `src/pages/InterviewAnalytics.tsx`<br>Aggregates from `interviews`, `mock_interview_sessions`, `question_practice_responses`<br>Charts for interview count, success rate, category performance | None | ✅ Covered in `analytics-calculations.test.ts` | None |
| UC-079 | Company Research | ✅ DONE | `src/components/interviews/CompanyResearchReport.tsx` (625 lines)<br>`supabase/functions/ai-interview-research/index.ts`<br>`interviews.company_research` (JSONB) | None | ⚠️ No dedicated test | Create `src/test/sprint3/uc079-company-research.test.tsx` |
| UC-080 | Interview Scheduling | ✅ DONE | `src/components/interviews/InterviewScheduler.tsx`<br>`interviews` table (scheduled_start, scheduled_end)<br>`calendar_integrations` table<br>Edge functions: calendar-oauth-start/callback/sync | Google Calendar OAuth requires setup (documented fallback: ICS export) | ✅ Has handler tests in `supabase/functions/calendar-sync/` | None |
| UC-081 | Interview Prep Checklist | ✅ DONE | `src/components/interviews/InterviewChecklistCard.tsx`<br>`interview_checklists` table (category, is_required, completed_at)<br>Completion percentage tracking | None | ⚠️ No test file | Create `src/test/sprint3/uc081-checklist.test.tsx` |
| UC-082 | Follow-up Templates | ✅ DONE | `src/components/interviews/InterviewFollowupTemplates.tsx`<br>`src/components/interviews/PostInterviewFollowup.tsx`<br>`interview_followups` table (type, status, template_subject/body, sent_at) | None | ⚠️ No test file | Create `src/test/sprint3/uc082-followup.test.tsx` |
| UC-083 | Interview Reminders | ✅ DONE | `src/components/interviews/InterviewReminders.tsx`<br>`notifications` table with RLS<br>`supabase/functions/send-daily-notifications/index.ts` (scheduled edge function) | None | ⚠️ No dedicated test | Create `src/test/sprint3/uc083-reminders.test.tsx` |
| UC-084 | Interview Performance Scoring | ✅ DONE | `interviews.outcome` column<br>`mock_interview_summaries.overall_score`<br>`question_practice_feedback.overall_score`<br>Analytics page aggregates | None (covered by UC-076, UC-077) | ✅ Covered in existing tests | None |
| UC-085 | Technical Challenge Prep | ✅ DONE | `src/pages/TechnicalPrep.tsx`<br>`src/pages/TechnicalChallengeDetail.tsx`<br>`technical_challenges`, `technical_practice_attempts` tables<br>Monaco editor integration | None | ⚠️ No test file | Create `src/test/sprint3/uc085-technical-prep.test.tsx` |
